...
// Phase 3: completed Cal.com booking measurement for Imigracao Brasileira
(() => {
  const register = () => {
    if (typeof window.Cal !== 'function' || window.__imigracaoCalSuccessRegistered) return;
    window.__imigracaoCalSuccessRegistered = true;
    window.Cal('on', {
      action:'bookingSuccessfulV2',
      callback:(event) => {
        let snapshot='';
        try{snapshot=JSON.stringify(event?.detail?.data||event?.detail||{}).toLowerCase()}catch{}
        const type=snapshot.includes('in-person-consultation')?'in_person':snapshot.includes('free-phone-consultation')?'free_phone':'appointment';
        const key=`imigracaoBooking:${type}:${new Date().toISOString().slice(0,10)}`;
        try{if(sessionStorage.getItem(key))return;sessionStorage.setItem(key,'1')}catch{}
        if(typeof gtag==='function')gtag('event','appointment_booked',{brand:'imigracao_brasileira',page_path:location.pathname,appointment_type:type,currency:type==='in_person'?'USD':undefined,value:type==='in_person'?50:undefined});
      }
    });
  };
  register(); setTimeout(register,500); setTimeout(register,1800);
})();
